import { modalSelector, showModal, stopWatchState } from "@/states";
...
export default function Bag() {
  const setModalState = useSetRecoilState(showModal);
  ...
  // stopwatch recoil state
  const setCompleteWatch = useSetRecoilState(stopWatchState);

  // 수강신청완료시 recoil stopWatchState 변경
  useEffect(() => {
    if (completeData.length === applyDummyData.length) {
      setCompleteWatch("pause");
      Swal.fire({
        icon: "success",
        title: "수강신청이 완료되었습니다.",
  ...
  const modalShow = useRecoilValue(modalSelector);
